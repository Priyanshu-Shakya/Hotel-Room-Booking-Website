...
function showSearchAlert(message, duration = 4000) {
...
// !Customer Visisted Feedback/Review by the Form
  document.getElementById("reviewForm").addEventListener("submit", function(e) {
    e.preventDefault();
    const form = e.target;

    // Rating
    const rating = form.querySelector('input[data-rating]:checked');
    if (!rating) {
      alert("Please rate your stay (1 to 5).");
      return;
    }

    // Likes
    const likes = form.querySelectorAll('input[data-likes]');
    const likedOne = Array.from(likes).some(cb => cb.checked);
    const likesError = document.getElementById("likesError");
    if (!likedOne) {
      likesError.classList.remove("d-none");
      return;
    } else {
      likesError.classList.add("d-none");
    }

    // Recommend
    const recommend = form.querySelector('input[data-recommend]:checked');
    if (!recommend) {
      alert("Please select whether you'd recommend us.");
      return;
    }
    ...
    setTimeout(() => {
      const modalInstance = bootstrap.Modal.getInstance(document.getElementById("customReviewModal"));
      modalInstance?.hide();
    }, 2000);
...
document.getElementById("reviewForm").addEventListener("submit", function(e) {
  ...
  // ✅ validate likes (custom inline error)
  const checkboxes = form.querySelectorAll('input[name="likes"]');
  const likedOne = Array.from(checkboxes).some(cb => cb.checked);
  const error = document.getElementById("likesError");
  ...
  // ✅ validate recommend
  const recommendSelected = form.querySelector('input[name="recommend"]:checked');
  if (!recommendSelected) {
    form.querySelector('input[name="recommend"]').focus();
    return;
  }
  ...
  setTimeout(() => {
    const modal = bootstrap.Modal.getInstance(document.getElementById("customReviewModal"));
    modal.hide();
  }, 2000);
